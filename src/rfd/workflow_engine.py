...
class GatedWorkflow:
        ...
        if state:
            ...
            if locked_by and locked_by != session_id:
                ...
                if locked_at:
                    ...
                    if datetime.now() - lock_time > timedelta(minutes=30):
                        ...
                        conn.close()
                        return False, f"Feature locked by session {locked_by} at {locked_at}"
            
            # Update lock
            conn.execute(
                "UPDATE workflow_state SET locked_by = ?, locked_at = ?, updated_at = ? WHERE feature_id = ?",
                (session_id, datetime.now().isoformat(), datetime.now().isoformat(), feature_id)
            )
        else:
            ...
            conn.execute(
                """INSERT INTO workflow_state 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (feature_id, WorkflowState.IDEATION.value, session_id, 
    ...
    def proceed_to_next(self, feature_id: str, session_id: str) -> Tuple[bool, str]:
        ...
        conn = sqlite3.connect(self.db_path)
        lock = conn.execute(
            "SELECT locked_by FROM workflow_state WHERE feature_id = ?",
            (feature_id,)
        ).fetchone()
        ...
        if not can_go:
            ...
            conn.execute(
                """INSERT INTO drift_log (feature_id, session_id, attempted_action, blocked_reason, timestamp)
                   VALUES (?, ?, ?, ?, ?)""",
                (feature_id, session_id, "proceed_to_next", reason, datetime.now().isoformat())
            )
            conn.commit()
            conn.close()
            return False, reason
    ...
    def add_query(self, feature_id: str, query: str) -> int:
        ...
        cursor = conn.execute(
            """INSERT INTO workflow_queries (feature_id, state, query, timestamp)
               VALUES (?, ?, ?, ?)""",
            (feature_id, current.value if current else None, query, datetime.now().isoformat())
        )
    ...
    def get_unresolved_queries(self, feature_id: str) -> List[Dict]:
        ...
        return [
            {"id": q[0], "query": q[1], "state": q[2], "timestamp": q[3]}
            for q in queries
        ]
    ...
    def block_drift(self, feature_id: str, action: str) -> bool:
        ...
        if action in drift_actions:
            ...
            conn.execute(
                """INSERT INTO drift_log (feature_id, session_id, attempted_action, blocked_reason, timestamp)
                   VALUES (?, ?, ?, ?, ?)""",
                (feature_id, "current", action, f"Action would cause drift from {current.value}", 
    ...
    def _validate_specification(self, feature_id: str) -> Tuple[bool, str]:
        ...
        spec_file = Path('specs') / f"*-{feature_id}" / 'spec.md'
    ...
    def _validate_planning(self, feature_id: str) -> Tuple[bool, str]:
        ...
        plan_file = Path('specs') / f"*-{feature_id}" / 'plan.md'
    ...
    def get_workflow_status(self, feature_id: str) -> Dict[str, Any]:
        ...
        drift_attempts = conn.execute(
            """SELECT COUNT(*) FROM drift_log WHERE feature_id = ?""",
            (feature_id,)
        ).fetchone()[0]
        
        # Get hallucination attempts
        hallucinations = conn.execute(
            "SELECT COUNT(*) FROM hallucination_log"
        ).fetchone()[0]
        ...
        status = {
            ...
            "checkpoints_passed": [
                {"state": c[0], "passed": c[1], "timestamp": c[2]}
    ...
    def enforce_linear_flow(self, feature_id: str, requested_action: str) -> Tuple[bool, str]:
        ...
        if required_state:
            if current != required_state:
                return False, f"Action '{requested_action}' requires state {required_state.value}, currently in {current.value}"
